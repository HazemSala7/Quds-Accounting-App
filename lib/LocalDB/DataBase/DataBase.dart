...
class CartDatabaseHelper {
  ...
  final Map<String, Map<String, String>> expectedSchemas = {
    'cart': {
      ...
      'ponus1': 'TEXT NOT NULL',
      'ponus2': 'TEXT NOT NULL',
  ...
  Future<List<HistoryModel>> getHistoriesRecords() async {
    final db = await database;
    final maps = await db!.query('history');
    return List.generate(
        maps.length, (i) => HistoryModel.fromDatabaseJson(maps[i]));
  }
  ...
  Future<List<CatchModel>> getAllCatchReceiptsQabds() async {
    final db = await database;
    final maps = await db!.query(
      'catch_receipts',
      where: 'qType = ?',
      whereArgs: ['qabd'],
      orderBy: 'id DESC', // 👈 order results descending
    );
  ...
  Future<List<CatchModel>> getCatchReceiptsByDateRange(
    ...
    final maps = await db!.query(
      ...
      whereArgs: [startDate, endDate, 'qabd'],
  ...
  Future<void> clearHistoriesTable() async {
    final db = await database;
